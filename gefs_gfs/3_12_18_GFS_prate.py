...
processed_steps_file = os.path.join(BASE_DIR_AVG, "processed_steps_3_12_18.txt")
error_log_file = os.path.join(BASE_DIR_AVG, "errors_3_12_18.txt")
...
gefs_members = ["03", "12", "18"]  # members to include
...
for step in forecast_steps:
    ...
    try:
        ...
        ax.set_title(
            (
                f"GFS + GEFS3 + GEFS12 + GEFS18 precip/prate & MSLP | FH{step_str} | "
                f"Valid {valid_utc:%Y-%m-%d %HZ} ({forecast_local_time} ET, {forecast_day})\n"
                f"Runs: GFS {run_hour}Z | GEFS03 {run_hour}Z | GEFS12 {run_hour}Z | GEFS18 {run_hour}Z\n"
                f"{agreement_text}"
            ),
            fontsize=7,
            pad=6,
        ...
        png_path = os.path.join(BASE_DIR_AVG, "png", f"3_12_18_prate_mslp_avg_{step_str}.png")

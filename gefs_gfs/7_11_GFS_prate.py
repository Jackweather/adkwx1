...
BASE_DIR_AVG = BASE_DIR / '7_11_GFS_OUTPUT'
...
for step in forecast_steps:
    ...
    try:
        ...
        ax.set_title(
            (
                f"GFS + GEFS7 + GEFS11 precip/prate & MSLP | FH{step_str} | "
                f"Valid {valid_utc:%Y-%m-%d %HZ} ({forecast_local_time} ET, {forecast_day})\n"
                f"Runs: GFS {run_hour}Z | GEFS07 {run_hour}Z | GEFS11 {run_hour}Z\n"
                f"{agreement_text}"
            ),
            fontsize=7,
            pad=6,
...
print("All GFS + GEFS7 + GEFS11 averages downloaded and plotted in AVG_PRATE!")

...
for step in forecast_steps:
    ...
    try:
        ...
        mesh = ax.contourf(Lon2d, Lat2d, avg_data, levels=prate_levels, cmap=cmap, norm=norm, extend='max', transform=ccrs.PlateCarree())
        mslp_contours = ax.contour(Lon2d, Lat2d, avg_mslp, levels=mslp_levels, colors='black', linewidths=0.5, transform=ccrs.PlateCarree())
        ...
        snow_mesh = ax.contourf(Lon2d, Lat2d, snow_prate_masked, levels=snow_levels, cmap=snow_cmap, norm=snow_norm, extend='max', transform=ccrs.PlateCarree(), alpha=0.85, zorder=(mesh.get_zorder() if 'mesh' in locals() else 4) + 1)
        ...
        mslp_contours_top = ax.contour(Lon2d, Lat2d, avg_mslp, levels=mslp_levels, colors='black', linewidths=0.6, transform=ccrs.PlateCarree(), zorder=210)
        ...
        fig.suptitle(f" plot1 estimated precip/prate & mslp — Run: {run_date} {run_hour}Z | Forecast: {step_str} ({forecast_local_time} EST, {forecast_day}){accuracy_str}", fontsize=8, fontweight='bold', y=0.80)

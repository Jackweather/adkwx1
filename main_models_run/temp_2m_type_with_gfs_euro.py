...
BASE_DIR = OUTPUT_ROOT / "EURO_GFS_T2M_OUTPUT"
GRIB_DIR = BASE_DIR / "grib"
PNG_DIR = BASE_DIR / "png"
LOG_FILE = BASE_DIR / "errors_euro_gfs_t2m.txt"
...
TEMP_LEVELS_F = [-30, -20, -10, 0, 10, 20, 32, 40, 50, 60, 70, 80, 90, 100]
TEMP_COLORS = [
    "#4b0082", "#6a5acd", "#4169e1", "#1e90ff",
    "#87cefa", "#c7f0ff", "#d9f7c8", "#9be564",
    "#4caf50", "#f7e35c", "#f9b233", "#ef6c00",
    "#d84315",
]
TEMP_CMAP = ListedColormap(TEMP_COLORS)
TEMP_NORM = BoundaryNorm(TEMP_LEVELS_F, TEMP_CMAP.N, clip=False)
...
def find_latest_icon_run():
    ...
    for offset in range(16):
        ...
        run_date = test_time.strftime("%Y%m%d")
        run_hour = f"{test_time.hour:02d}"
        test_url = build_icon_field_url(run_date, run_hour, "t_2m", "000", "T_2M")
        try:
            response = requests.head(test_url, timeout=20, allow_redirects=True)
        except requests.RequestException:
            continue
...
def kelvin_to_fahrenheit(values):
    return (np.asarray(values, dtype=np.float32) - 273.15) * 9.0 / 5.0 + 32.0


def open_gfs_t2m(path):
    ...
    try:
        ...
        field = kelvin_to_fahrenheit(dataset[variable_name].squeeze().values)
...
def open_ecmwf_fields(path):
    ...
    t2m_dataset = xr.open_dataset(path, engine="cfgrib", filter_by_keys={"typeOfLevel": "heightAboveGround", "level": 2})
    try:
        msl = as_data_array(
        ...
        variable_name = "t2m" if "t2m" in t2m_dataset.data_vars else next(iter(t2m_dataset.data_vars))
        t2m = as_data_array(
            kelvin_to_fahrenheit(t2m_dataset[variable_name].squeeze().values),
            t2m_dataset["latitude"].values,
            t2m_dataset["longitude"].values,
            "ecmwf_t2m_f",
        )
        return msl, t2m
    finally:
...
def open_gdps_mslp(path):
    field = open_gdps_scalar_field(path)
    return field / 100.0


def open_gdps_t2m(path):
    field = open_gdps_scalar_field(path)
    field.values[:] = kelvin_to_fahrenheit(field.values)
    field.name = "gdps_t2m_f"
    return field
...
def plot_average_fields(avg_t2m, avg_mslp, available_runs, forecast_hour, step_str):
    gfs_run = available_runs[0][1]
    valid_utc, local_time, local_day = format_valid_time(gfs_run, forecast_hour)
    lon2d, lat2d = np.meshgrid(avg_t2m.longitude.values, avg_t2m.latitude.values)
    ...
    for region_name in ACTIVE_REGION_NAMES:
        ...
        temp_mesh = ax.contourf(
            lon2d,
            lat2d,
            avg_t2m.values,
            levels=TEMP_LEVELS_F,
            cmap=TEMP_CMAP,
            norm=TEMP_NORM,
            extend="both",
            transform=ccrs.PlateCarree(),
        ...
        cbar_width = 0.76
        cbar_height = 0.018
        cbar_ax_temp = fig.add_axes([cbar_left, cbar_bottom, cbar_width, cbar_height])
        temp_bar = fig.colorbar(temp_mesh, cax=cbar_ax_temp, orientation="horizontal")
        temp_bar.set_ticks([-20, 0, 32, 50, 70, 90])
        temp_bar.set_label("Avg 2m temperature (F)", fontsize=7)
        temp_bar.ax.tick_params(labelsize=6)

        ax.set_title(
            (
                f"{model_text} 2m temp & MSLP | {REGION_LABELS[region_name]} | FH{step_str} | "
        ...
        output_path = PNG_DIR / region_name / f"euro_gfs_t2m_mslp_avg_{step_str}.png"
...
def main():
    ...
    for forecast_hour in forecast_steps:
        ...
        try:
            ...
            download_file(
                build_gfs_url(gfs_run_date, gfs_run_hour, step_str, "var_TMP=on&lev_2_m_above_ground=on"),
                gfs_t2m_path,
                f"GFS 2m temp FH{step_str}",
            )
            ...
            if icon_available:
                ...
                download_bz2_file(
                    build_icon_field_url(icon_run_date, icon_run_hour, "t_2m", step_str, "T_2M"),
                    icon_t2m_path,
                    f"ICON 2m temp FH{step_str}",
                )
            ...
            if gdps_available:
                ...
                download_file(
                    build_gdps_field_url(gdps_run_date, gdps_run_hour, step_str, "AirTemp_AGL-2m"),
                    gdps_t2m_path,
                    f"GDPS 2m temp FH{step_str}",
                )

            gfs_mslp = open_gfs_mslp(gfs_mslp_path)
            gfs_t2m = open_gfs_t2m(gfs_t2m_path)

            if ecmwf_available:
                ecmwf_mslp, ecmwf_t2m = open_ecmwf_fields(ecmwf_path)
                ecmwf_mslp = regrid_to_gfs(ecmwf_mslp, gfs_mslp)
                ecmwf_t2m = regrid_to_gfs(ecmwf_t2m, gfs_mslp)
            else:
                ecmwf_mslp = None
                ecmwf_t2m = None
            ...
            if gdps_available:
                ...
                gdps_t2m = regrid_to_gfs(open_gdps_t2m(gdps_t2m_path), gfs_mslp)
            else:
                gdps_mslp = None
                gdps_t2m = None
            ...
            if icon_available:
                ...
                icon_t2m = remap_icon_to_gfs(
                    kelvin_to_fahrenheit(open_icon_scalar_field(icon_t2m_path, ["t2m", "2t"])),
                    icon_indexer,
                    gfs_mslp,
                    "icon_t2m_f",
                )

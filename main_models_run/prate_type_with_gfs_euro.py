...
def main():
    ...
    for forecast_hour in forecast_steps:
        ...
        try:
            ...
            gfs_prate_path.unlink(missing_ok=True); gc.collect()
            gfs_mslp_path.unlink(missing_ok=True); gc.collect()
            gfs_csnow_path.unlink(missing_ok=True); gc.collect()
            ecmwf_path.unlink(missing_ok=True); gc.collect()
            icon_pmsl_path.unlink(missing_ok=True); gc.collect()
            icon_tp_path.unlink(missing_ok=True); gc.collect()
            icon_snow_gsp_path.unlink(missing_ok=True); gc.collect()
            icon_snow_con_path.unlink(missing_ok=True); gc.collect()
            gdps_mslp_path.unlink(missing_ok=True); gc.collect()
            gdps_precip_path.unlink(missing_ok=True); gc.collect()
            gdps_snow_path.unlink(missing_ok=True); gc.collect()
